...
async function api(req, env, url) {
  ...
  if (url.pathname === '/api/admin/login' && req.method === 'POST') {
    const b = await req.json();
    if (!env.ADMIN_PASSWORD || String(b.password || '') !== env.ADMIN_PASSWORD) return json({ ok: false, error: 'Sai mật khẩu.' }, 401);
    return json({ ok: true, token: await sha256(env.ADMIN_PASSWORD) });
  }
