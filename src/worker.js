// PATCH AUTH v2: robust ADMIN_PASSWORD normalization and token compatibility
...
function cleanSecret(v) {
  // Loại bỏ BOM, xuống dòng và khoảng trắng vô tình khi dán Secret trên Cloudflare.
  return String(v ?? '').replace(/^\uFEFF/, '').trim();
}

async function adminOk(req, env) {
  let supplied = String(req.headers.get('X-Admin-Token') || '').trim();
  if (supplied.toLowerCase().startsWith('bearer ')) supplied = supplied.slice(7).trim();

  const saved = cleanSecret(env.ADMIN_PASSWORD);
  if (!supplied || !saved) return false;

  const hashed = await sha256(saved);
  // Chấp nhận token SHA-256 chuẩn; đồng thời hỗ trợ bản frontend cũ gửi mật khẩu trực tiếp.
  return supplied === hashed || supplied === saved;
}
...
async function api(req, env, url) {
  ...
  if (url.pathname === '/api/admin/login' && req.method === 'POST') {
    ...
    // Chuẩn hóa Secret và dữ liệu gửi lên để tránh lỗi BOM/xuống dòng khi dán mật khẩu.
    const savedPassword = cleanSecret(env.ADMIN_PASSWORD);
    const inputPassword = cleanSecret(b.password ?? b.pass ?? b.adminPassword);
    ...
    const savedHash = await sha256(savedPassword);
    // Hỗ trợ cả mật khẩu gốc lẫn frontend cũ đã băm SHA-256 trước khi gửi.
    const valid = inputPassword === savedPassword || inputPassword.toLowerCase() === savedHash;
    if (!valid) return json({ ok: false, error: 'Sai mật khẩu.' }, 401);

    return json({ ok: true, token: savedHash });
  }
  ...
  if (url.pathname === '/api/admin/unlock' && req.method === 'POST') {
    ...
    await env.DB.prepare(
      "INSERT INTO unlock_tokens(token,customer_id,unlock_type,expires_at,status) VALUES(?,?,?,?, 'issued')"
