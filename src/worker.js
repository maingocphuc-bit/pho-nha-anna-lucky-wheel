...
const PRIZES = [
  { name: '1 Tô Phở Ăn Miễn Phí', odds: 2, special: true, cap: 2 },
...
async function api(req, env, url) {
  ...
  if (url.pathname === '/api/admin/redeem' && req.method === 'POST') {
    ...
    const b = await req.json();
    const codeVal = String(b.code || '').trim();
    const reward = await env.DB.prepare(
      "SELECT reward_code,redeemed,created_at," +
      "CASE WHEN datetime(created_at) < datetime('now','-2 days') THEN 1 ELSE 0 END AS expired " +
      "FROM plays WHERE reward_code=?"
    ).bind(codeVal).first();

    if (!reward) return json({ ok: false, error: 'Không tìm thấy mã QR.' }, 404);
    if (Number(reward.expired) === 1) {
      return json({ ok: false, error: 'Mã QR đã quá hạn 2 ngày và không còn được áp dụng.' }, 410);
    }
    if (Number(reward.redeemed) === 1) {
      return json({ ok: false, error: 'Mã QR này đã được đổi quà trước đó.' }, 409);
    }

    const r = await env.DB.prepare(
      "UPDATE plays SET redeemed=1,redeemed_at=datetime('now') " +
      "WHERE reward_code=? AND redeemed=0 AND datetime(created_at) >= datetime('now','-2 days')"
    ).bind(codeVal).run();

    return json({ ok: r.meta.changes === 1, error: r.meta.changes === 1 ? undefined : 'Mã QR không còn hợp lệ.' });
  }
  ...
  if (url.pathname === '/api/reward' && req.method === 'GET') {
    ...
    const r = await env.DB.prepare(
      "SELECT plays.reward_code,plays.prize_name,plays.redeemed,plays.redeemed_at,plays.created_at,customers.name," +
      "CASE WHEN datetime(plays.created_at) < datetime('now','-2 days') THEN 1 ELSE 0 END AS expired " +
    ...
    if (Number(r.expired) === 1) {
      return json({ ok: false, expired: true, error: 'Mã QR đã quá hạn 2 ngày và không còn được áp dụng.' }, 410);
    }
